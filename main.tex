...
By our convention, let $\boldsymbol{x}_1,...,\boldsymbol{x}_N$ be the observations and $\boldsymbol{e}_1,...,\boldsymbol{e}_N$ be the artificially generated noise data with known distribution $\pi\left(\boldsymbol{e}\right)$. The $\widehat{\boldsymbol{\theta}^\star}$ estimate is then defined as 
\begin{align}
    \widehat{\boldsymbol{\theta}^\star} =& \argmax_{\boldsymbol{\theta}^\star} \pazocal{L}^{\mathrm{NC}}\left(\boldsymbol{\theta}^\star \right) \\= &\argmax_{\boldsymbol{\theta}^\star}\frac{1}{2N} \sum_{i=1}^N \ln{h(\boldsymbol{x}_i, \boldsymbol{\theta}^\star)} + \ln{\left(1 - h(\boldsymbol{e}_i, \boldsymbol{\theta}^\star)\right)}
\end{align}
where function $h$ is in the form
\begin{align}
    h(.) = \frac{1}{1 + \exp(-\ln q_{\boldsymbol{\theta}^\star}\left(\boldsymbol{.}\right) + \ln \pi\left(\boldsymbol{.}\right))}
\end{align}

It may be noted that equation 4.14 also appears in supervised learning and is called binary cross-entropy loss.  It is actually a special case of cross-entropy itself. Thus, it is used for classification between two classes. 
...
\begin{figure}[h]
	...
	{{\includegraphics[width=8.0cm]{images/tikzit_image1} }}%
	\subfloat[Multiple instance learning]
	{{\includegraphics[width=8.0cm]{images/tikzit_image3} }}%
