...
\includegraphics[width=3cm,height=3cm,keepaspectratio]{images/cvut}
\par\end{center}%
\end{minipage}%
...
\begin{minipage}[c]{3cm}%
\noindent \begin{center}
\includegraphics[width=3cm,height=3cm,keepaspectratio]{images/fjfi}
\par\end{center}%
\end{minipage}
...
for his expert guidance and patience during this and previous academic years. I would like to thank my family and my friends for all the support I was given. 
...
In general, prediction error evaluated on the training data for increasing model complexity approaches zero. On the other hand, prediction error evaluated on the testing data for increasing model complexity is (from certain point) increasing as well. The typical scenario is illustrated in Figure \ref{fig:Prediction_error}.
 \begin{figure}[h]
	\centering
	\includegraphics[width=16.0cm]{images/PE3.pdf}
...
\chapter{Discriminative modeling}
\chapter{Generative modeling}
...
\begin{equation}
	...
	\end{equation}
where subscript $\boldsymbol{\theta}$ emphasizes that $\pi_{\boldsymbol{\theta}}(x)$ is our approximative density we get to control. Note that the first term does not depend on $\boldsymbol{\theta}$ and therefore minimazing either Cross-Entropy or Kullback-Leibler divergence is equivalent. Finally, by minimizing with respect to $\pi_{\boldsymbol{\theta}}(x)$ we obtain
\begin{equation}
...
\end{equation}
In practise it is dealt with discrete data, so $ \E_{x\sim \psi}\left[\log \pi_{\boldsymbol{\theta}}\left(x\right)\right] $ takes the form
\begin{equation}
\E_{x\sim \psi}\left[\log \pi_{\boldsymbol{\theta}}\left(x\right)\right] = \sum_{i=1}^N\psi(x_i)\log \pi_{\boldsymbol{\theta}}\left(x_i)\right)
\end{equation}
...
Suppose we have to estimate a model that is specified by an non-normalized probability density function $q^0_{\boldsymbol{\theta}}\left(\boldsymbol{x}\right)$. Thus, we introduce the another parameter $c$ among the estimated parameters $\boldsymbol{\theta}$. For clarity, the symbol $\boldsymbol{\theta}^\star=\lbrace \boldsymbol{\theta}, c\rbrace$ is introduced for the set of estimated parameters including $c$. 
Using this notation, the following equality then holds 
\begin{equation}
\ln q_{\boldsymbol{\theta}^\star}\left(\boldsymbol{x}\right) = \ln q^0_{\boldsymbol{\theta}}\left(\boldsymbol{x}\right) + c
\end{equation}
which means that the newly introduced parameter $c$ is an estimate of the negative logarithm of the normalization constant $Z(\boldsymbol{\theta})$.\\
As the name of the subsection suggests, we use noise to estimate. 
By our convention, let $\boldsymbol{x}_1,...,\boldsymbol{x}_N$ be the observations and $\boldsymbol{e}_1,...,\boldsymbol{e}_N$ be the artificially generated noise data with known distribution $\psi\left(\boldsymbol{e}\right)$. The $\widehat{\boldsymbol{\theta}^\star}$ estimate is then defined as 
\begin{align}
    \widehat{\boldsymbol{\theta}^\star} =& \argmax_{\boldsymbol{\theta}^\star} \pazocal{L}^{\mathrm{NC}}\left(\boldsymbol{\theta}^\star \right) \\= &\argmax_{\boldsymbol{\theta}^\star}\frac{1}{2N} \sum_{i=1}^N \ln{\sigma_{\bt^\star}(\boldsymbol{x}_i}) + \ln{\left(1 - \sigma_{\bt^\star}(\boldsymbol{e}_i)\right)}
\end{align}
where $\sigma_{\bt^\star}$ is logistic function
\begin{align}
    \sigma_{\bt^\star}(\boldsymbol{x}) = \frac{1}{1 + \exp\left(-G_{\boldsymbol{\theta}^\star}(\boldsymbol{x})\right)}
\end{align}
and finally $G_{\bt^\star}$ is the difference of the log-likelihoods of $q_{\boldsymbol{\theta}^\star}$ and    $\psi$, hence
\begin{align}
G_{\bt^\star}(\boldsymbol{x}) = \ln q_{\boldsymbol{\theta}^\star}\left(\boldsymbol{x}\right) - \ln \psi\left(\boldsymbol{x}\right).
\end{align}

It may be noted that equation 4.15 also appears in supervised learning and is called binary cross-entropy loss.  It is actually a special case of cross-entropy itself. Thus, it is used for classification between two classes. This gives an intuitive insight into how noise-contrastive estimation really works. By comparing data and noise, the model is learned, so this method can be called learning by comparison.
\subsubsection{Experiment}
To test this approach, we perform a simple experiment.There are a total of $N=100$ i.i.d. one dimensional observations $x$ from an unknown distribution that is assumed to be non-normalized and Gaussian. It is therefore of the form
\begin{equation}
q_{\bt^\star}(x) =  \exp\left(-\frac{1}{2}\cdot\frac{(x-\mu)^2}{\sigma^2} + c\right)
\end{equation}
where $\bt^\star = \{\mu, \sigma, c\}$. Next, we artificially generate $N=100$ noise data $e$, which is again easiest to do using a Gaussian distribution. Which means that it can be chosen, for example
\begin{equation}
\psi(e) =  \frac{1}{\sqrt{2\pi 10}}\exp\left(-\frac{1}{2}\cdot\frac{e^2}{10}\right).
\end{equation}
At this point it is possible to construct a function $\pazocal{L}^{\mathrm{NC}}\left(\boldsymbol{\theta}^\star \right)$ that is minimized using stochastic gradient descent. The following figure shows the training process and the comparison between the estimated distribution and the true one.
\begin{figure}[h]
	\centering
	\subfloat[Training loss function]
	{{\includegraphics[width=8.0cm]{images/NCE_loss.pdf} }}%
	\subfloat[Comparison of true and estimated pdf]
	{{\includegraphics[width=8.0cm]{images/NCE_results.pdf} }}%
	\caption{Results of the NCE experiment.}%
	\label{NCE_experiment}%
\end{figure}
As can be seen in the figure \ref{NCE_experiment}, this approach works quite well and for more observations results would be even better.
...
\end{equation}
where the choice of parameter $\tau$ is based on the fact that we would like to have a non--informative prior, thus $\tau$ should be adequately high. If the value of $\tau$ is high, data are spread very far from their expected value.  Substituting Equation \eqref{eq:tauprior} and \eqref{eq:linregpdf} to the \eqref{eq:chainrule} results to
\begin{equation}
...
\begin{figure}[h] 
	...
	{{\includegraphics[width=8.0cm]{images/tau1.pdf} }}%
	\subfloat[Estimated model for $\tau=10$ and $s=11$.]
	{{\includegraphics[width=8.0cm]{images/tau10.pdf} }}%
	\
	\subfloat[Estimated model for $\tau=100$ and $s=11$.]
	{{\includegraphics[width=8.0cm]{images/tau100.pdf} }}%,
	\subfloat[Estimated model for $\tau=1000$ and $s=11$.]
	{{\includegraphics[width=8.0cm]{images/tau1000.pdf} }}%
	\
	\subfloat[Estimated model for $\tau=100000$ and $s=11$.]
	{{\includegraphics[width=8.0cm]{images/tau100000.pdf} }}%,
	\subfloat[Estimated model for $\tau=1000000$ and $s=11$.]
	{{\includegraphics[width=8.0cm]{images/tau1000000.pdf} }}%
...
\begin{figure}[h]
	...
	{{\includegraphics[width=8.0cm]{images/n2.pdf} }}%
	\subfloat[Estimated model for $\tau=100$ and $s-1=4$.]
	{{\includegraphics[width=8.0cm]{images/n4.pdf} }}%
	\
	\subfloat[Estimated model for $\tau=100$ and $s-1=5$.]
	{{\includegraphics[width=8.0cm]{images/n5.pdf} }}%,
	\subfloat[Estimated model for $\tau=100$ and $s-1=6$.]
	{{\includegraphics[width=8.0cm]{images/n6.pdf} }}%
	\
	\subfloat[Estimated model for $\tau=100$ and $s-1=8$.]
	{{\includegraphics[width=8.0cm]{images/n8.pdf} }}%,
	\subfloat[Estimated model for $\tau=100$ and $s-1=10$.]
	{{\includegraphics[width=8.0cm]{images/n10.pdf} }}%
...
\begin{figure}[h]
	...
	{{\includegraphics[width=8.0cm]{images/tikzit_image1.pdf} }}%
	\subfloat[Multiple instance learning]
	{{\includegraphics[width=8.0cm]{images/tikzit_image3.pdf} }}%
...
\begin{figure}[h]
	\centering
	\includegraphics[width=16.0cm, trim={4cm 4.0cm 0.5cm 3.5cm},clip]{images/nn.pdf}
...
\begin{figure}[h]
	...
	{{\includegraphics[width=8.0cm]{images/KFCV_Musk4.pdf} }}%
	\subfloat[CV for dataset Musk2.]
	{{\includegraphics[width=8.0cm]{images/KFCV_Musk2.pdf} }}%
	\
	\subfloat[CV for dataset Fox.]
	{{\includegraphics[width=8.0cm]{images/KFCV_Fox.pdf} }}%
	\subfloat[CV for dataset Tiger.]
	{{\includegraphics[width=8.0cm]{images/KFCV_Tiger.pdf} }}%
...
\begin{figure}[h]
	...
	{{\includegraphics[width=8.2cm]{images/HDGM_Musk3.pdf} }}%
	\subfloat[HDGM for dataset Musk2.]
	{{\includegraphics[width=8.2cm]{images/HDGM_Musk4.pdf} }}%
	\
	\subfloat[HDGM for dataset Fox.]
	{{\includegraphics[width=8.2cm]{images/HDGM_Fox.pdf} }}%
	\subfloat[HDGM for dataset Tiger.]
	{{\includegraphics[width=8.2cm]{images/HDGM_Tiger.pdf} }}
...
\begin{figure}[h]
	...
	{{\includegraphics[width=8.2cm]{images/supervisedvsHDGM0_5_Musk1.pdf} }}%
	\subfloat[HDGM: $\alpha=0.5$ vs Discriminative; Musk2.]
	{{\includegraphics[width=8.2cm]{images/supervisedvsHDGM0_5_Musk2.pdf} }}%
	\
	\subfloat[HDGM: $\alpha=0.5$ vs Discriminative; Fox.]
	{{\includegraphics[width=8.2cm]{images/supervisedvsHDGM0_5_Fox.pdf} }}%
	\subfloat[HDGM: $\alpha=0.5$ vs Discriminative; Tiger.]
	{{\includegraphics[width=8.2cm]{images/supervisedvsHDGM0_5_Tiger.pdf} }}
