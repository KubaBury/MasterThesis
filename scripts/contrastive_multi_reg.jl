...
σ = 0.6
n=10 #degree of the polynom
#x1 = rand(Normal(1,0.04), 5)
#x2 = rand(Normal(2,0.05), 5)
#x = vcat(x1,x2) 
#y = 3 .*x .+ 1 + rand(Normal(0, σ^2 ), 10)
α = 0:0.1:1
Θ = zeros(n+1,length(α)) #preallocated space 
θ = zeros(n+1) #preallocated space 

τ = 100. #regularization parameter

#define model
f(θ,x,y) =  .-(((y'.- hcat([(x).^o for o = 0:n]...)*θ).^2) ./(2σ^2)) .- ((x.^2)./(2*(τ^2))) 
...
#Plots
gr()
scatter(x,y, label=L"$\mathrm{data}$", legend=:topleft,markershape=:cross, color=:black, markersize=6, )
x3 = 0.8:0.01:2.2
W2 = [(x3).^i for i = 0:n]
X2 = hcat(W2...)
Y2 = X2*Θ
