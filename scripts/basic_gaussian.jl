...
"""
function Gaussianlogpdf(u::Vector{Float64}, μ::Float64, σ::Float64, c::Float64)
    if σ >= 0.0
...
variance σ and normalization constant c. Note that σ should be greater than 0.
"""
function G(u::Vector{Float64}, μ::Float64, σ::Float64, c::Float64)
    if σ >= 0.0 
...
variance σ and normalization constant c. Note that σ should be greater than 0.
"""
function h(u::Vector{Float64}, μ::Float64, σ::Float64, c::Float64) 
    h = 1 ./ (1 .+ exp.(-G(u, μ, σ, c)))
    return h
end
